...
async function fetchData(url: string) {
    let resp = await fetchJsonp("https://interface.sina.cn/news/wap/" + url, {timeout: 10000});
    return await resp.json();
}

async function fetchFixedData() {
    let resp = await fetchJsonp("https://joyqi.com/proxy.php?url="
        + encodeURIComponent("https://c.m.163.com/ug/api/wuhan/app/data/list-total"), {timeout: 10000}),
